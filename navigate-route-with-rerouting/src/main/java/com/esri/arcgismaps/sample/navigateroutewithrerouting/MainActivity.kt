...
import com.esri.arcgismaps.sample.navigateroutewithrerouting.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
     ...
     */
    private suspend fun setUpMapRoute() {
        ...
        routeTask.load().onFailure {
            showError("Failed to load route task: ${it.message}")
        }
        // get the loaded default route parameters
        val routeParameters = routeTask.createDefaultParameters().getOrElse {
            return showError("Error creating default parameters:${it.message}")
        }.apply {
        ...
        val conventionCenter = Point(-117.160386727, 32.706608, SpatialReference.wgs84())
        ...
        val aerospaceMuseum = Point(-117.146679, 32.730351, SpatialReference.wgs84())
        ...
        route?.routeGeometry?.let { mapView.setViewpointGeometry(it, 25.0) }
        // enable navigation
        navigateButton.isEnabled = true


        // start the route navigation
        startNavigation(routeResult, routeParameters)
    }
    ...
    private suspend fun startNavigation(
        ...
        lifecycleScope.launch {
            routeTracker?.let { updateVoiceGuidance(it) }
        }

        // handle route tracking status changes
        lifecycleScope.launch {
            routeTracker?.trackingStatus?.collect {
                if (it != null) {
        ...
        if (routeTask.getRouteTaskInfo().supportsRerouting) {
            ...
            routeTracker?.enableRerouting(reroutingParameters)?.getOrElse {
                showError("Error enabling rerouting")
            }

            lifecycleScope.launch {
                mapView.locationDisplay.autoPanMode.collect {
                    // turn the recenter button on or off when
                    // the location display changes to or from navigation mode
                    recenterButton.isEnabled = it != LocationDisplayAutoPanMode.Navigation
                }
            }
            ...
            simulatedLocationDataSource.setLocationsWithPolyline(
                Geometry.fromJsonOrNull(polylineJSON) as Polyline,
                simulationParameters
            )

            lifecycleScope.launch {
                // provide the route tacker with the current location of the simulated datasource
                simulatedLocationDataSource.locationChanged.collect {
                    routeTracker?.trackLocation(it)
                }
            }
            ...
            lifecycleScope.launch {
                routeTracker?.rerouteCompleted?.collect { trackingStatusResult ->
                    ...
                    directionManeuvers.addAll(trackingStatus.routeResult.routes[0].directionManeuvers)
                    ...
                    lifecycleScope.launch {
                        routeTracker?.newVoiceGuidance?.collect{
                            //(speakDirectionListener)
                        }
                    }
    ...
    private fun updateTrackingStatus(status: TrackingStatus) {
        ...
        if (status.isOnRoute && !status.isCalculatingRoute) {
